...
export default function UploadPMPOPBox() {
    ...
    const validateFileName = (
        ...
        //  FIXED: Block SPK files (karena ini halaman Form PM POP)
        ...
        if (hasSpkKeyword) {
            return {
                ...
                blockReason:
                    ...
                    'Halaman ini HANYA untuk dokumen Form PM POP.',
            };
        }
        ...
        if (lowerName.includes(' spk ') || lowerName.startsWith('spk ') || lowerName.endsWith(' spk')) {
            return {
                ...
                blockReason:
                    ...
                    'Jika ini Form PM POP, pastikan nama file mengandung kata "PM POP", "PMPOP", atau "Preventive Maintenance".',
            };
        }

        //  Validasi positif: file yang mengandung keyword Form PM POP
        const pmpopKeywords = [
            'pm pop',
            'pm_pop',
            'pmpop',
        ...
        const hasPMPOPKeyword = pmpopKeywords.some((keyword) => lowerName.includes(keyword));
        ...
        return {
            ...
            warning:
                ...
                'Pastikan ini adalah dokumen Form PM POP.\n\n' +
    ...
    const handleSubmit = () => {
        ...
        const toastId = toast.loading('Uploading dan validasi dokumen Form PM POP...', {
            classNames: {
        ...
        router.post('/documents/pmpop', formData, {
            forceFormData: true,
            onSuccess: (page) => {
                ...
                setTimeout(() => {
                    toast.info('💡 Tips: Dokumen akan divalidasi dalam beberapa detik', {
                        duration: 3000,
                        description: 'Jika file bukan Form PM POP, akan otomatis dihapus dan diberi notifikasi.',
                        classNames: {
    ...
    return (
        <div className="mt-2 flex flex-col items-center px-4">
            <div
                className={`w-full max-w-md rounded-lg border-2 border-dashed ${isDragging ? 'border-cyan-400 bg-gray-800' : 'border-blue-400 bg-gray-900'} flex min-h-[250px] flex-col justify-center p-6 text-center transition-colors duration-200`}
                ...
                {!selectedFile ? (
                    <>
                        ...
                        <p className="mb-2 text-sm font-semibold text-white">
                            {isDragging ? 'Lepaskan file di sini' : 'Drag & drop PDF Form PM POP to upload'}
                        </p>
                        <p className="mb-4 text-xs text-gray-400">Hanya untuk: Form PM POP (Preventive Maintenance - Point of Presence)</p>
                        <button
                            onClick={handleClick}
                            className="mx-auto rounded-full bg-gradient-to-r from-blue-400 to-cyan-400 px-6 py-2 text-white shadow transition hover:brightness-110"
                        >
                            Select PDF Form PM POP
                        </button>
            ...
            {/* Enhanced Info Box */}
            <div className="mt-4 w-full max-w-md rounded-lg border border-blue-400/30 bg-blue-900/30 p-4">
                <div className="mb-3">
                    <p className="mb-2 text-xs font-semibold text-blue-200">Dokumen yang BOLEH diupload di halaman ini:</p>
                    <ul className="ml-4 space-y-1 text-xs text-blue-300">
                        <li> Form PM POP (Preventive Maintenance - Point of Presence)</li>
                    </ul>
                </div>
